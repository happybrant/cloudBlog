...
public class RedisKeyUtil {
  ...
  /**
   * 某个实体（帖子、评论/回复）的获赞 like:entity:entityType:entityId -> set(userId)
  ...
  /**
   * 某个用户的获赞数量 like:user:userId -> int
   *
  ...
  /**
   * 某个用户关注的实体 followee:userId:entityType -> zset(entityId, now) 以当前关注的时间进行排序
   *
   * @param userId
   ...
   *
   * @param owner 用户进入登录页面的时候，由于此时用户还未登录，无法通过 id 标识用户 随机生成一个字符串，短暂的存入 cookie，使用这个字符串来标识这个用户
   * @return
   */
